...
class Network(nn.Module):
    def __init__(self, N_STATES, HIDDEN_LAYER, N_ACTIONS):
        nn.Module.__init__(self)
        # self.l1 = nn.Linear(N_STATES, HIDDEN_LAYER)  
        # self.l1.weight.data.normal_(0, 0.1)      # weight initialization
        # self.l2 = nn.Linear(HIDDEN_LAYER, N_ACTIONS) 
        # self.l2.weight.data.normal_(0, 0.1)      # weight initialization
        
        self.l1 = nn.Linear(N_STATES, N_ACTIONS)
...
class DQN_Agent:
    def __init__(self, N_STATES = 5, HIDDEN_LAYER=3, N_ACTIONS=4, 
                 EPS_START=0.9, EPS_END=0.05, EPS_DECAY=80, 
                 GAMMA=0.9, LR=0.01, BATCH_SIZE=32, MEM_CAP=600):
        ### Hyper Parameters ###
        self.N_STATES = N_STATES           # input size
        self.HIDDEN_LAYER = HIDDEN_LAYER   # NN hidden layer size
        self.N_ACTIONS = N_ACTIONS         # output size
        
        self.EPS_START = EPS_START  # e-greedy threshold start value
        self.EPS_END = EPS_END      # e-greedy threshold end value
        self.EPS_DECAY = EPS_DECAY  # e-greedy threshold decay rate
        ...
        self.MEM_CAP = MEM_CAP  # memory capacity
